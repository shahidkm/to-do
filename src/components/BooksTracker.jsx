...
import { BookOpen, Plus, X, Check, Star, TrendingUp, Pencil } from 'lucide-react';
...
export default function BooksTracker() {
  ...
  const save = async () => {
    ...
    setForm({ title: '', author: '', total_pages: '', status: 'reading', rating: 0 });
    setAdding(false);
    await load();
  };
  ...
  const startEdit = (book) => {
    setForm({ title: book.title, author: book.author || '', total_pages: book.total_pages || '', status: book.status, rating: book.rating || 0 });
    setEditing(book.id);
    setAdding(true);
          ...
          {adding && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => { setAdding(false); setEditing(null); setForm({ title: '', author: '', total_pages: '', status: 'reading', rating: 0 }); }} className="px-5 py-3 rounded-xl text-slate-500"
